...
with open(path.join(FILE_DIR, 'lwe_plugin_provider_huggingface_hub', 'version.py')) as f:
    version = re.match(r'^__version__ = "([\w\.]+)"$', f.read().strip())[1]

setup(
    name="lwe-plugin-provider-huggingface-hub",
    version=version,
    ...
    entry_points={
        "lwe_plugins": [
            "lwe_plugin_provider_huggingface_hub = lwe_plugin_provider_huggingface_hub.plugin:ProviderHuggingfaceHub"
        ]
    },
)
